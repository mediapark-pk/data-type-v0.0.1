...
{
    std::cerr << "Critical Error in: " << assertion << ", File '" << file << "' in line " << line
              << "." << std::endl;
    exit(-1);
}
...
static std::string bc_add_positive(
    const char* lhs,
    int lint,
    int ldot,
    int lfrac,
...
static std::string bc_sub_positive(
    const char* lhs,
    int lint,
    int ldot,
    int lfrac,
...
static std::string bc_mul_positive(
    const char* lhs,
    int lint,
    int ldot,
    int lfrac,
...
static std::string bc_div_positive(
    const char* lhs,
    int lint,
    int ldot,
    int lfrac,
...
{
    if (lsign > 0 && rsign > 0) {
        return bc_add_positive(
            lhs, lint, ldot, lfrac, lscale, rhs, rint, rdot, rfrac, rscale, scale, 1);
    }

    if (lsign > 0 && rsign < 0) {
        if (bc_comp(lhs, lint, ldot, lfrac, lscale, rhs, rint, rdot, rfrac, rscale, 1000000000) >= 0) {
            return bc_sub_positive(
                lhs, lint, ldot, lfrac, lscale, rhs, rint, rdot, rfrac, rscale, scale, 1);
        } else {
            return bc_sub_positive(
                rhs, rint, rdot, rfrac, rscale, lhs, lint, ldot, lfrac, lscale, scale, -1);
        }
    }
...
std::string BCMath::bcdiv(const std::string& lhs, const std::string& rhs, int scale)
{
    ...
    if (bc_parse_number(lhs, lsign, lint, ldot, lfrac, lscale) < 0) {
        std::cerr << "First parameter \"" << lhs.c_str() << "\" in function bcdiv is not a number"
                  << std::endl;
        return ZERO;
    }
    ...
    if (bc_parse_number(rhs, rsign, rint, rdot, rfrac, rscale) < 0) {
        std::cerr << "Second parameter \"" << rhs.c_str() << "\" in function bcdiv is not a number"
                  << std::endl;
        return ZERO;
    }

    return bc_div_positive(
        lhs.c_str(), lint, ldot, lfrac, lscale, rhs.c_str(), rint, rdot, rfrac, rscale, scale, lsign * rsign);
}
...
std::string BCMath::bcmod(const std::string& lhs, const std::string& rhs)
{
    ...
    int lsign, lint, ldot, lfrac, lscale;
    if (bc_parse_number(lhs, lsign, lint, ldot, lfrac, lscale) != 0) {
        std::cerr << "First parameter \"" << lhs.c_str() << "\" in function bcmod is not an integer"
                  << std::endl;
        return ZERO;
    }
    ...
    if (bc_parse_number(rhs, rsign, rint, rdot, rfrac, rscale) != 0) {
        std::cerr << "Second parameter \"" << rhs.c_str()
                  << "\" in function bcmod is not an integer" << std::endl;
        return ZERO;
    }
    ...
    if (rdot - rint > 18 || mod == 0) {
        std::cerr << "Second parameter \"" << rhs.c_str()
                  << "\" in function bcmod is not a non zero integer less than 1e18 by absolute "
                     "value"
                  << std::endl;
        return ZERO;
    }
...
std::string BCMath::bcpow(const std::string& lhs, const std::string& rhs)
{
    ...
    int lsign, lint, ldot, lfrac, lscale;
    if (bc_parse_number(lhs, lsign, lint, ldot, lfrac, lscale) != 0) {
        std::cerr << "First parameter \"" << lhs.c_str() << "\" in function bcpow is not an integer"
                  << std::endl;
        return ZERO;
    }
    ...
    if (bc_parse_number(rhs, rsign, rint, rdot, rfrac, rscale) != 0) {
        std::cerr << "Second parameter \"" << rhs.c_str()
                  << "\" in function bcpow is not an integer" << std::endl;
        return ZERO;
    }
    ...
    if (rdot - rint > 18 || (rsign < 0 && deg != 0)) {
        std::cerr << "Second parameter \"" << rhs.c_str()
                  << "\" in function bcpow is not a non negative integer less than 1e18"
                  << std::endl;
        return ZERO;
    }
...
std::string BCMath::bcadd(const std::string& lhs, const std::string& rhs, int scale)
{
    ...
    int lsign, lint, ldot, lfrac, lscale;
    if (bc_parse_number(lhs, lsign, lint, ldot, lfrac, lscale) < 0) {
        std::cerr << "First parameter \"" << lhs.c_str() << "\" in function bcadd is not a number"
                  << std::endl;
        return bc_zero(scale);
    }
    ...
    int rsign, rint, rdot, rfrac, rscale;
    if (bc_parse_number(rhs, rsign, rint, rdot, rfrac, rscale) < 0) {
        std::cerr << "Second parameter \"" << rhs.c_str() << "\" in function bcadd is not a number"
                  << std::endl;
        return bc_zero(scale);
    }

    return bc_add(
        lhs.c_str(), lsign, lint, ldot, lfrac, lscale, rhs.c_str(), rsign, rint, rdot, rfrac, rscale, scale);
}
...
std::string BCMath::bcsub(const std::string& lhs, const std::string& rhs, int scale)
{
    ...
    int lsign, lint, ldot, lfrac, lscale;
    if (bc_parse_number(lhs, lsign, lint, ldot, lfrac, lscale) < 0) {
        std::cerr << "First parameter \"" << lhs.c_str() << "\" in function bcsub is not a number"
                  << std::endl;
        return bc_zero(scale);
    }
    ...
    int rsign, rint, rdot, rfrac, rscale;
    if (bc_parse_number(rhs, rsign, rint, rdot, rfrac, rscale) < 0) {
        std::cerr << "Second parameter \"" << rhs.c_str() << "\" in function bcsub is not a number"
                  << std::endl;
        return bc_zero(scale);
    }
...
std::string BCMath::bcmul(const std::string& lhs, const std::string& rhs, int scale)
{
    ...
    int lsign, lint, ldot, lfrac, lscale;
    if (bc_parse_number(lhs, lsign, lint, ldot, lfrac, lscale) < 0) {
        std::cerr << "First parameter \"" << lhs.c_str() << "\" in function bcmul is not a number"
                  << std::endl;
        return ZERO;
    }
    ...
    int rsign, rint, rdot, rfrac, rscale;
    if (bc_parse_number(rhs, rsign, rint, rdot, rfrac, rscale) < 0) {
        std::cerr << "Second parameter \"" << rhs.c_str() << "\" in function bcmul is not a number"
                  << std::endl;
        return ZERO;
    }

    return bc_mul_positive(
        lhs.c_str(), lint, ldot, lfrac, lscale, rhs.c_str(), rint, rdot, rfrac, rscale, scale, lsign * rsign);
}
...
int BCMath::bccomp(const std::string& lhs, const std::string& rhs, int scale)
{
    ...
    int lsign, lint, ldot, lfrac, lscale;
    if (bc_parse_number(lhs, lsign, lint, ldot, lfrac, lscale) < 0) {
        std::cerr << "First parameter \"" << lhs.c_str() << "\" in function bccomp is not a number"
                  << std::endl;
        return 0;
    }
    ...
    int rsign, rint, rdot, rfrac, rscale;
    if (bc_parse_number(rhs, rsign, rint, rdot, rfrac, rscale) < 0) {
        std::cerr << "Second parameter \"" << rhs.c_str() << "\" in function bccomp is not a number"
                  << std::endl;
        return 0;
    }
...
std::string BCMath::bcround(const std::string& lhs, int scale)
{
    ...
    int lsign, lint, ldot, lfrac, lscale;
    if (bc_parse_number(lhs, lsign, lint, ldot, lfrac, lscale) < 0) {
        std::cerr << "First parameter \"" << lhs.c_str() << "\" in function bcround is not a number"
                  << std::endl;
        return 0;
    }
